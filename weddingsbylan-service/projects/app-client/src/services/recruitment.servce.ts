...
class RecruitmentService extends CRUDService<IRecruitment, number> {
    Filter = async (filter: any, signal?: AbortSignal) => {
        const data = await this.TryGet<IRecruitment[]>("/", {
            signal,
            params: {
                filter
            }
    ...
    Random = async (count: number, locale: string, signal?: AbortSignal) => {
        const data = await this.TryGet<IRecruitment[]>("/random", {
            signal,
            params: {
                count, locale
            }
...
export const recruitmentService = new RecruitmentService("recruitments")