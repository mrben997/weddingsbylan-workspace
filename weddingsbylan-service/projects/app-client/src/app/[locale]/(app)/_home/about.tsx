...
import { IAboutForm } from '@/admin-react-app/pages/settings/setting.form.types'

const defaultValue: Partial<IAboutForm> = {
  Title: 'About Lan Le',
  Content:
...
interface IHomeAboutProps {
  data?: IAboutForm[] | null
}

const HomeAbout: FC<IHomeAboutProps> = (props) => {
  const data = props.data ? props.data[0] : undefined
  return (
    <section className='section section-bg home-about-area' {...getEditModeKey('AboutImage')}>
      <div className='section-content right'>
        <div className='section-content-haft'>
          <div className='flex-column' {...getEditModeKey('About')}>
            <h1 className='typography-h1'>{data?.Title || defaultValue.Title}</h1>
