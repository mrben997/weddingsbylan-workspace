    ...
    render() {
      ...
      return (
        <Box sx={{ display: 'flex', flexDirection: 'column', border: '2px solid #1976d2', borderRadius: '4px' }}>
          <input name={this.props.name?.toString()} hidden value={this.state.result} onChange={() => {}} />
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', padding: '5px' }}>
              ...
              <Button disabled={this.state.isUpload} onClick={this.onUpload}>
                Upload
              </Button>
              <Button disabled={this.state.isUpload} onClick={this.onReset}>
                Reset
              </Button>
              {options.size ? (
                <>
            ...
            <Box>{this.state.isUpload ? <CircularProgress size={20} /> : <></>}</Box>
          </Box>
          <Typography sx={{ textAlign: 'center', width: '100%' }} color='error' variant='caption'>
            {this.state.message}
          </Typography>
